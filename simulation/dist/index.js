...
const simulateCars = () => {
    const cycle = async (pathObj) => {
        while (true) {
            ...
            const [x, y] = path[i];
            db.query(`INSERT INTO rides (car_id, location, path)
...
class Customer {
    ...
    async updateDB() {
        return db.query(`INSERT INTO customers (name, active, location, destination)
      VALUES (
        '${this.name}',
        ${this.active},
        ...
        while (true) {
            ...
            if (this.active !== newActive) {
                ...
                if (newActive) {
                    ...
                    getDestination.send({ name: this.name, location });
                }
                else {
                    this.active = false;
                    this.location = null;
