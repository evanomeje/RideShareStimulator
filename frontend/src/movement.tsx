...
export const getNextCoordIndex = (currX: number, currY: number, path: number[][]): number => {
...
export const advanceCoord = (curr: number, next: number, increment: number): number => {
  if (next > curr) {
    curr = curr + increment;
...
export const getTurnDistance = (curr: number, target: number) => ({
  distClockwise: target > curr && target <= 360 ? target - curr : 360 - curr + target,
  distCounterclockwise: target >= 0 && target < curr ? curr - target : curr + 360 - target,
});
