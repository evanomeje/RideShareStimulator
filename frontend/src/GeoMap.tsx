...
const loadData = async (previousUpdateAtRef, setCars, setRefreshing) => {
  while (true) {
    ...
    for (const ride of rides) {
      ...
      cars.push({ id: car_id, path, actual: [parseInt(x), parseInt(y)] });
    }
    setCars(cars);
    setRefreshing(false);
    await wait(fetchInterval);
...
const GeoMap = () => {
  ...
  for (let [key, color] of obstaclesMap) {
    const [x, y] = key.split(':');
    obstacleElems.push(
      <rect key={`${x}:${y}`} width={squareSize} height={squareSize}
        x={x * squareSize} y={y * squareSize} fill={color} stroke={color} />
    );
  }

  const carElems = cars.map(({ id, actual, path }) => (
    <Car key={id} actual={actual} path={path} />
  ));

  const customerElems = customers.map(({ location }) => {
    const [x, y] = location.split(':');
    return (
      <CustomerIcon key={`${x}:${y}`}
